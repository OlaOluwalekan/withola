...
export async function generateAiSummary(readme: string): Promise<{ summary?: string; error?: string }> {
  try {
    ...
    if (!apiKey) {
      console.warn("GEMINI_API_KEY environment variable is not set. Returning a fallback summary for development.");
      return { summary: "This is a fallback AI generated summary because the GEMINI_API_KEY environment variable is missing. " + "x".repeat(150) };
    }
